...
pub async fn send(
    ...
    let fee_per_byte = common::get_fee_per_byte(network).await;
    ...
    let own_address = Address::from_str(&own_address)
        .unwrap()
        .require_network(common::transform_network(network))
...
async fn build_p2pkh_spend_tx(
    ...
    let mut total_fee = 0;
    loop {
        let (transaction, _prevouts) = common::build_transaction_with_fee(
            own_utxos,
            own_address,
            dst_address,
            amount,
